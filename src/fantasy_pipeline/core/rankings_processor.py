...
class RankingsProcessor:
        ...
        if self.league_type in ["weekly", "ros"]:
            try:
                auto_scrape_if_needed(
                    week=self.week, league_type=self.league_type, data_path=data_path, force=False, verbose=verbose
                )
        ...
        for key, df in dataframes.items():
            ...
            if key in column_mappings:
                ...
                if len(df.columns) == len(expected_cols):
                    ...
                    if verbose:
                        print(
                            f"   ⚠️  Column count mismatch for {key}: expected {len(expected_cols)}, got {len(df.columns)}"
                        )
    ...
    def _create_consolidated_rankings(
        ...
        for key, df in dataframes.items():
            ...
            if available_columns:
                ...
                df_rank = df_rank.merge(df[join_columns], on="PLAYER ID", how="left", suffixes=("", f"_{key}"))
    ...
    def _calculate_average_rankings(self, df_rank: pd.DataFrame, verbose: bool) -> pd.DataFrame:
        ...
        if self.league_type not in ["weekly", "ros"]:
            ...
            if rk_columns_for_avg:
                ...
                if verbose:
                    print(
                        f"   ✓ Calculated avg_RK from {len(rk_columns_for_avg)} ranking columns: {rk_columns_for_avg}"
                    )
            else:
                df_rank["avg_RK"] = None
                if verbose:
        ...
        if pos_rank_columns_for_avg:
            ...
            if verbose:
                print(
                    f"   ✓ Calculated avg_POS RANK from {len(pos_rank_columns_for_avg)} position ranking columns: {pos_rank_columns_for_avg}"
                )
        else:
    ...
    def _merge_weekly_ros_data(
        self, df_rank: pd.DataFrame, dataframes: Dict[str, pd.DataFrame], verbose: bool
    ) -> pd.DataFrame:
        ...
        if "fpts-data" in dataframes:
            ...
            fpts_columns_to_keep = ["PLAYER ID"] + [
                col for col in numeric_cols if col not in excluded_cols and col != "PLAYER ID"
            ]
    ...
    def _merge_historical_stats(self, df_rank: pd.DataFrame, latest_dir: str, verbose: bool) -> pd.DataFrame:
        ...
        historical_stats_files = [
            f for f in os.listdir(latest_dir) if f.startswith("rankings_ready_historical_stats_") and f.endswith(".csv")
        ]
        ...
        if historical_stats_files:
            ...
            try:
                ...
                hist_data_count = (
                    df_rank[df_rank.columns[df_rank.columns.str.startswith("HIST_")]].notna().any(axis=1).sum()
                )
    ...
    def _organize_final_dataframe(self, df_rank: pd.DataFrame, verbose: bool) -> pd.DataFrame:
        ...
        if self.league_type in ["weekly", "ros"]:
            ...
            fpts_data_columns = [
                ...
                if col not in reserved_columns and not any(pattern in col for pattern in reserved_patterns)
            ]
    ...
    def _archive_processed_files(self, data_path: str, files: List[str], raw_archive_dir: str, verbose: bool):
...
def process_weekly_rankings(week: Optional[int] = None, **kwargs) -> "str | pd.DataFrame":

...
def calculate_weekly_trends(
    ...
    _calculate_outlier_adjusted_average(
        df_processed, week_columns, outlier_method, outlier_percentile, outlier_n_games, verbose
    )
...
def _identify_week_columns(df: pd.DataFrame, verbose: bool) -> List[str]:
    ...
    if verbose:
        print(
            f"   ✓ Found {len(week_columns)} week columns: {week_columns[:5]}{'...' if len(week_columns) > 5 else ''}"
        )
...
def _calculate_outlier_adjusted_average(
    df: pd.DataFrame, week_columns: List[str], method: str, percentile: float, n_games: int, verbose: bool
) -> None:
...
def _print_weekly_summary(df: pd.DataFrame) -> None:
    ...
    if "Pos" in df.columns:
        ...
        for pos, stats in pos_stats.iterrows():
            print(
                f"     {pos}: 1H={stats['FIRST_HALF_AVG']:.1f}, 2H={stats['SECOND_HALF_AVG']:.1f}, "

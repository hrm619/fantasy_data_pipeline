...
- ⚠️ **`HIST_*` is the 2024 season and is NOT auto-refreshed.** The checked-in
  `rankings_ready_historical_stats_20250822_1855.csv` was generated 2025-08-22 with `ff-stats --season`
  defaulting to **2024**, and `data/fpts historical/combined_data.csv` holds seasons **2014–2024 only** —
  the 2025 season was never ingested. So on a 2026 board these columns are **two seasons stale**. There is
  no fetcher for this data (the `s20XX.xlsx` files are manual drops); refreshing means ingesting the new
  season first, then re-running `ff-stats --season <year>`. Don't assume `HIST_*` describes last season.
